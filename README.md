...
## SafePromptGuard v4.1 - Eval Baseline

v4.1은 Gitleaks/TruffleHog 연동 전에 현재 detector와 policy 엔진의 기준선을 측정하는 eval 시스템을 추가합니다. `backend/eval/dataset.jsonl`은 secrets, PII, internal info, benign prompt, edge case 각 10개씩 총 50개 샘플을 포함하며, `backend/scripts/evaluate_scan.py`는 detected, masked, blocked, safe_prompt null, secret leakage, overall_action 정확도를 측정합니다.
